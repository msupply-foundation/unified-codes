...
export type RxNavTermType = "IN" | "PIN" | "MIN" | "SCDC" | "SCDF" | "SCDG" | "SCD" | "GPCK" | "BN" | "SBDC" | "SBDF" | "SBDG" | "SBD" | "BPCK" | "PSN" | "SY" | "TMSY" | "DF" | "ET" | "DFG";
...
export class RxNavDataSource extends RESTDataSource {
    ...
    async getInteractions(rxcui: string): Promise<IDrugInteraction[]> {
      const body: IRxNavInteractionResponseBody = await this.get(RxNavDataSource.paths.interactions, { rxcui });
      const interactions = body.interactionTypeGroup.flatMap((interactionTypeGroup: IRxNavInteractionTypeGroup) => {
        const { sourceName: source } = interactionTypeGroup;
        return interactionTypeGroup.interactionType.flatMap((interactionType: IRxNavInteractionType) => {
          return interactionType.interactionPair.flatMap((interactionPair: IRxNavInteractionPair) => {
