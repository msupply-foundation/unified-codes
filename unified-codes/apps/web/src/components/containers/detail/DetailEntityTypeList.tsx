...
export const DetailEntityTypeList: DetailEntityTypeList = ({ parent, entities }) => {
    ...
    const entityTypeGroups = React.useMemo(() => entities.reduce((acc, child) => {
        const { type } = child;
    ...
    const entityTypeListItems = React.useMemo(() => Object.keys(entityTypeGroups).map(type => {
        const entities = entityTypeGroups[type];
        return <DetailEntityTypeListItem key={type} type={type} parent={parent} entities={entities}/>;
    }), [entityTypeGroups]);
