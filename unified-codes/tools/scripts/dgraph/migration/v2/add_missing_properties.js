...
const inputJSONFilename = process.argv[2] || '../../data/v1/products.json';
const inputCSVFilename = process.argv[2] || '../../data/v2/products.csv';
const outputFilename = process.argv[3] || '../../data/v2/products_merged.csv';
...
const processItem = (item) => {
  ...
  if (item.has_property) {
    item.has_property.forEach((property) => {
      const key = `${item.type}_${property.type}`;
      if (!uniqueProps[key]) {
        uniqueProps[key] = 0;
      }
      uniqueProps[key] += 1;
...
const migrate = async () => {
  ...
  if (!inputJSONFilename || !outputFilename) {
  ...
  try {
    fileHandle = await fs.open(inputJSONFilename, 'r');
    await fs.readFile(fileHandle).then(parseData); // .then(writeFile);
    console.info(uniqueProps);
